...
const paths: Record<string, ReactNode> = {
  ...
  /* 变更（git）：左侧一列 +/- 记号 + 右侧文件行 —— "有增有删的清单"（不用 Δ 之类抽象符号） */
  diff: (
    <>
      <path d="M10 6h9M10 12h9M10 18h9" />
      <path d="M3.6 4.6v2.8M2.2 6h2.8" />
      <path d="M2.2 18h2.8" />
    </>
  ),
  /* 浏览器：地球 + 经线（比"窗口"更能表达"访问外部站点"） */
  globe: (
    <>
      <circle cx="12" cy="12" r="9" />
      <path d="M3 12h18" />
      <path d="M12 3c2.4 2.6 3.7 5.6 3.7 9S14.4 18.4 12 21c-2.4-2.6-3.7-5.6-3.7-9S9.6 5.6 12 3z" />
    </>
  ),
  /* ── 右侧面板开关（2026-09-23）── */
  /* 圆角栏体 + 右侧分隔线 = "内容在左、栏体在右"。
     刻意**不复用** `folder` / `diff`：那两枚表达的是"栏里装什么"（视图标签用），
     这一枚表达的是"这一栏要不要占屏幕" —— 开合与内容是两个语义，图标也不能共用。 */
  panelRight: (
    <>
  ...
  /* 在系统文件管理器中定位（"打开到外面去"）—— 用外链箭头而不是文件夹图标：
     文件夹图标在这个界面里已经表示"目录"了，两个语义不能共用一枚图标 */
  externalLink: (
    <>
      <path d="M14 4h6v6" />
      <path d="M20 4 11 13" />

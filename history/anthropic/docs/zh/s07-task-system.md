...
1. `创建3个任务："Setup project"、"Write code"、"Write tests"。让它们按顺序相互依赖。`
2. `列出所有任务并显示依赖关系图`
3. `完成任务1，然后列出任务以查看任务2是否已解除阻塞`
4. `为重构创建一个任务看板：parse -> transform -> emit -> test，其中 transform 和 emit 可以在 parse 之后并行运行`
